...
class Inspection(Base):
    ...
    def product_type(self) -> str | None:
        if self.working_memory and isinstance(self.working_memory, dict):
            pt = self.working_memory.get("product_type")
            if pt:
                return str(pt)
        if self.golden_reference:
            meta = getattr(self.golden_reference, "meta", None) or {}
            if isinstance(meta, dict) and meta.get("product_type"):
                return str(meta.get("product_type"))
        return None
