...
| **Day 4 (Aug 17)** | `backend/app/routers/inspections.py` — POST /inspect (upload + trigger pipeline), GET /inspections (list), GET /inspections/{id}, GET /inspections/{id}/stream (SSE live pipeline progress streaming via sse-starlette) | `backend/app/shared/evidence_store.py` — Evidence Store banao (append-only, audit trail) |
