...
| Day | Anil | Disha |
|:---|:---|:---|
| **Day 1** | [x] Repo create, `.gitignore`, `README.md`, `LICENSE`, `.env.example`, `Makefile`, poora folder structure banao | [x] `requirements.txt` — sab dependencies list karo, `venv` setup karo |
| **Day 2** | [x] `backend/app/core/config.py` — pydantic-settings se environment variables load karo | [x] `backend/app/core/database.py` — SQLAlchemy 2.0 async engine + asyncpg PostgreSQL session setup karo |
| **Day 3** | [x] `backend/app/core/security.py` — JWT token creation, password hashing (bcrypt) | [x] `backend/app/core/exceptions.py` — custom error classes banao (NotFound, Unauthorized, ValidationError, etc.) |
...
| **Day 6** | [ ] Dono milke: `backend/app/core/__init__.py` + sab `__init__.py` files banao, poora core/ test karo — server start hota hai, .env load hota hai, logging kaam karta hai | [ ] Dono milke: Core test karo |
...
| **Day 6** | [ ] Dono milke: sab 9 agents individually test karo — sample ROI crops pe run karke evidence output verify karo | [ ] Dono milke: Agent test verify |
